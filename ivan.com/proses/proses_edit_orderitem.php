...
if (!empty($_POST['edit_orderitem_validate'])) {
    $select = mysqli_query($conn, "SELECT * FROM tb_list_order WHERE menu=$menu && kode_order = $kode_order && kode_order=$kode_order && id_list_order!=$id");
    if (mysqli_num_rows($select) > 0) {
        $message = '<script>alert("Item yang dimasukkan telah ada, silahkan masukkan item lain");
        ...
        if ($query) {
            $message = '<script>alert("Order berhasil diedit");
            window.location="../?x=orderitem&order=' . $kode_order . '&meja=' . $meja . '&pelanggan=' . $pelanggan . '"</script>';
        } else {
