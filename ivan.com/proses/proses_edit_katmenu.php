...
if (!empty($_POST['input_katmenu_validate'])) {
    ...
    if (mysqli_num_rows($select) > 0) {
        $message = '<script>alert("Kategori menu yang dimasukkan telah ada, silahkan gunakan kategori menu lain");
        ...
        if ($query) {
            $message = '<script>alert("Kategori berhasil di edit");
        window.location="../katmenu"</script>';
        } else {
            $message = '<script>alert("Kategori gagal diedit");
        window.location="../katmenu"</script>';
        }
    }
}
